...
function initPageTransitions() {
  ...
  function initSmoothScroll(container) {
    ...
    // Smooth scrolling for arrow-container
    const arrowContainer = document.querySelector(".arrow-container");
    if (arrowContainer) {
      arrowContainer.addEventListener("click", function () {
        const targetElement = document.getElementById("karthik-intro");
...
function initScript() {
  ...
  initScrolltriggerChatbotToggle();
...
function initScrolltriggerChatbotToggle() {
  ...
  let hasInteractedWithChat = false; // Tracks if the user has interacted with the chatbot
  ...
  function showToggleButton() {
    toggleChatbotButton.classList.add("show");
    toggleChatbotButton.classList.remove("hide");
  }

  // Function to hide the toggle button
  function hideToggleButton() {
    toggleChatbotButton.classList.add("hide");
    toggleChatbotButton.classList.remove("show");
  }

  // Event listener for Locomotive Scroll
  scroll.on("scroll", (instance) => {
    const scrollY = instance.scroll.y; // Get Locomotive's scroll position
    updateToggleVisibility(scrollY);
  });

  // Event listener for chatbot interactions (send button click)
  sendButton.addEventListener("click", () => {
    hasInteractedWithChat = true; // Mark as interacted
    updateToggleVisibility(scroll.instance.scroll.y); // Ensure visibility logic is updated
  });

  // Event listener for chatbot interactions (Enter key press)
  ...
  toggleChatbotButton.addEventListener("click", () => {
    ...
    chatbot.classList.add("show"); // Add animation class if needed

    hideToggleButton(); // Hide the toggle button when chatbot is open
    setTimeout(() => {
      toggleChatbotButton.style.display = "none";
    }, 400); // Match CSS transition duration
  });

  // Event listener for closing chatbot (minimizing)
  chatbotClose.addEventListener("click", () => {
    chatbot.style.display = "none"; // Hide chatbot
    chatbot.classList.remove("show"); // Remove animation class if needed

    hasInteractedWithChat = true; // Mark as interacted to ensure it remains visible
    toggleChatbotButton.style.display = "flex";
    showToggleButton(); // Immediately show the toggle button when minimizing the chatbot
  });
}
